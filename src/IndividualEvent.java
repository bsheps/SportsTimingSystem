...
public class IndividualEvent implements EventInterface {
	Queue<Racer> WaitingToRace = (Queue<Racer>) new LinkedList<Racer>(),
			inTheRace = (Queue<Racer>) new LinkedList<Racer>(), finishers = (Queue<Racer>) new LinkedList<Racer>();
	 ...
	 * either the start or finish fields of the racer (odd channels start, even
	 * finish)
	 * 
	 * @param chNum
	...
	public void trigger(int chNum) {
		if (chNum == 1) { // odd means start time
			if (WaitingToRace.size() == 0) {
				...
				inTheRace.add(n);
				// print.printThis(" Channel " + chNum+ " triggered at "
				// +Time.time2formattedString(Time.getCurrentTime()) + " for racer: noName");
			} else {
				Racer x = WaitingToRace.remove();
				x.startRace(Time.getCurrentTime());
				// print.printThis(" Channel " + chNum+ " triggered at "
				// +Time.time2formattedString(Time.getCurrentTime()) + " for racer: "
				// +x._bibNum);
				inTheRace.add(x);
				x = null;
			}
			...
			if (inTheRace.size() == 0)
				; // print.printThis("Channel " + chNum +" triggered at " +
					// Time.time2formattedString(Time.getCurrentTime()) +"--ERROR: No more racers");
			else {
